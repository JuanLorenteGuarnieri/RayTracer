...
Cilindro::Cilindro(G_O _centro, float _altura, float _radio, Color _color, Textura _tx, BRDF _brdf)
{
    brdf = _brdf;
    centro = _centro;
    altura = _altura;
...
Cilindro::Cilindro(G_O _centro, float _altura, float _radio, Color _color, Textura _tx, BRDF _brdf, int tipoLuz, Color _potencia)
{
    brdf = _brdf;
    centro = _centro;
    altura = _altura;
...
bool Cilindro::interseccion(float &t, Color &_color, const Rayo rayo, const int tipoColor, G_O &normal)
{
    ...
    float a = (rayo.direccion.x * rayo.direccion.x) + (rayo.direccion.z * rayo.direccion.z);
    float b = 2 * (A * rayo.direccion.x + B * rayo.direccion.z);
    float c = A * A + B * B - radio * radio;
    ...
    if ((r > centro.y) && (r < centro.y + altura))
    {
        if (tipoColor == 1)
        {
...
bool Cilindro::interseccion(float &t, Color &_color, const Rayo rayo, const int tipoColor)
{
...
bool Cilindro::interseccion(float &t, Color &_color, const Rayo rayo)
{
    float A = rayo.punto.x - centro.x;
    float B = rayo.punto.z - centro.z;
    float D = altura - rayo.punto.y + centro.y;
